...
<#macro blockrender data=[] field="" title="" measure="none">
	<div class="rblock">
		<div class="ritem">
			...
			<span class="rcell rtb">${title}</span>
		</div>
		<div class="rbox">
	<#if data?size gt 0>
	<#local MAX=0>
	<#list data?sort_by(field) as item>
			...
			<#else>
				...
				<span class="rcell rzb">&nbsp;</span>
			<#if measure == "money">
				<span class="rcell rtb"><@wanyuan val=item[field]/></span>
			<#elseif measure == "rate">
				<span class="rcell rtb"><@percent val=item[field]/></span>
			<#else>
				<span class="rcell rtb"><@number val=item[field]/></span>		
			</#if>
			</div>
	</#list>
	<#else>
...
<#if orgType=="head">
	<@blockrender data=details field=index title="毛利额" measure="money"/>
<#elseif orgType=="area">
	<@blockrender data=details1 field=index title="毛利额" measure="money"/>
</#if>
</div>
...
<#if orgType=="head">
	function toLoadStores(areaId, areaName){
		...
		$('#RANKING_DLG .dialog-panel').load(report.maingoodsUrl1, savedParams, function(){
			$('#RANKING_DLG').unblock();
		});
	}
<#elseif orgType=="area">
	if(report.maingoodsUrl1 != null){
